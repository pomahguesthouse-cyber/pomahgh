...
export const Footer = memo(({ editorMode = false }: FooterProps) => {
  ...
  const description = hotelSettings?.description || "Penginapan Murah dan Nyaman di Kota Semarang.";
      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 mb-6 sm:mb-8">
          ...
          <div className="text-center sm:text-left">
            <h3 className="text-lg sm:text-xl md:text-2xl font-bold mb-2 sm:mb-4 font-sans">{hotelName}</h3>
            <p className="text-xs sm:text-sm md:text-base text-primary-foreground/80">{description}</p>
          </div>
            ...
            <div className="flex gap-3 sm:gap-4 justify-center sm:justify-start">
              {hotelSettings?.facebook_url && (
                <a
                  ...
                  className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-primary-foreground/10 hover:bg-primary-foreground/20 flex items-center justify-center transition-colors"
                >
        ...
        <div className="border-t border-primary-foreground/20 pt-4 sm:pt-6 md:pt-8 text-center text-primary-foreground/60">
          <p className="text-xs sm:text-sm">
            &copy; {new Date().getFullYear()} {hotelSettings?.hotel_name || "Pomah Guesthouse"}. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
