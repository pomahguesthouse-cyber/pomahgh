...
export const FacilityCard = ({
  ...
  return <div className="flex flex-col items-center text-center p-4 sm:p-6 rounded-lg bg-card hover:shadow-lg transition-shadow">
...
// ---------- Skeleton Card ----------
export const FacilitySkeleton = () => <div className="animate-pulse p-6 rounded-lg bg-card">
...
const FacilitiesHero = () => {
  ...
  return <div className="relative mb-12 w-full overflow-hidden">
      <Carousel opts={{
      loop: true
    }} plugins={[autoplayPlugin]} className="w-full" setApi={setApi}>
        <CarouselContent>
          {slides.map(slide => <CarouselItem key={slide.id}>
              <div className="relative w-full overflow-hidden">
                <OptimizedImage src={slide.image_url} alt={slide.title || "Facility"} width={1200} height={600} priority={slides.indexOf(slide) === 0} placeholder="blur" className="w-full aspect-[2/1]" />
                
                {/* Title & Subtitle */}
                {(slide.title || slide.subtitle) && <div className="absolute inset-0 flex flex-col justify-center items-center text-center px-4">
                    {slide.title && <motion.h3 initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
                ...
                duration: 0.6
                    ...
                    {slide.subtitle && <motion.p initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
        {slides.map((_, index) => <button key={index} className={`w-2 h-2 rounded-full transition-all ${index === currentSlide ? "bg-white w-8" : "bg-white/50"}`} onClick={() => {}} />)}
      </div>
    </div>;
};
...
export const Amenities = () => {
  ...
  const renderSkeletons = () => <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
      {[1, 2, 3].map(i => <FacilitySkeleton key={i} />)}
    </div>;
  if (isLoading) {
    return <section id="amenities" className="py-20 px-4 bg-background">
  ...
  return <section id="amenities" className="pt-0 pb-20 bg-background">
      <FacilitiesHero />
      <div className="container mx-auto px-4">
        ...
        once: true
          ...
          {isEditorMode ? <EditableText widgetId="amenities" field="title" value={title} as="h2" className="text-2xl sm:text-3xl md:text-4xl font-cinzel font-semibold text-foreground mb-4" /> : <h2 className="text-2xl sm:text-3xl mb-4 md:text-3xl text-primary font-bold font-serif" style={getElementStyles('amenities-title')}>
              {title}
            </h2>}
          
          {/* Line with widget styling */}
          <div className="h-1 mx-auto mb-4 sm:mb-6 bg-primary" style={{
          width: lineStyle.width || '96px',
          height: lineStyle.height || '4px',
          backgroundColor: lineStyle.backgroundColor || undefined
        }} />
          
          {isEditorMode ? <EditableText widgetId="amenities" field="description" value={description} as="p" className="text-sm sm:text-base text-muted-foreground max-w-2xl mx-auto px-4" /> : <p className="text-sm sm:text-base text-muted-foreground max-w-2xl mx-auto px-4" style={getElementStyles('amenities-description')}>
              {description}
            </p>}
        </motion.div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {facilities.map(item => <FacilityCard key={item.id} icon={item.icon_name} title={item.title} description={item.description} />)}
        </div>
      </div>
    </section>;
};