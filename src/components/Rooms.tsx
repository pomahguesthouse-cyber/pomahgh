...
export const Rooms = () => {
  ...
  if (isLoading) {
    return <section id="rooms" className="py-20 px-4 bg-secondary/30">
  ...
  return <>
        ...
        <div className="container mx-auto">
          ...
          delay: 4000
            ...
            <CarouselContent className="-ml-2 md:-ml-4">
              ...
              const hasPromo = room.promo_price && room.promo_start_date && room.promo_end_date && new Date() >= new Date(room.promo_start_date) && new Date() <= new Date(room.promo_end_date);
              const displayPrice = room.final_price || room.price_per_night;
              return <CarouselItem key={room.id} className="pl-2 md:pl-4 md:basis-1/2 lg:basis-1/3">
                    <Card className="overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 h-full">
                      <div className="relative h-48 sm:h-56 md:h-64 overflow-hidden group">
                        {images.length > 1 ? <Carousel className="w-full h-full" plugins={[Autoplay({
                      delay: 3000
                    })]}>
                            <CarouselContent>
                              {images.map((image, index) => <CarouselItem key={index}>
                                   <img src={image} alt={`${room.name} - Photo ${index + 1}`} className="w-full h-48 sm:h-56 md:h-64 object-cover transition-transform duration-500 group-hover:scale-110" />
                                </CarouselItem>)}
                            </CarouselContent>
                        ...
                        {hasPromo && <div className="absolute top-2 right-2 z-10">
                      ...
                      <CardContent className="p-3 sm:p-4 md:p-6">
                        <div className="flex justify-between items-start mb-2 sm:mb-3">
                          <div className="flex-1 min-w-0 pr-2">
                            ...
                            {room.virtual_tour_url && <Badge variant="secondary" className="mb-2 text-xs">
                          ...
                          <div className="text-right flex-shrink-0">
                            ...
                            {hasPromo && <p className="text-xs line-through text-muted-foreground">
                        ...
                        return <div key={index} className="flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-1.5 bg-primary/10 text-primary rounded-full text-xs sm:text-sm" title={feature.label}>
          ...
          <div className="flex justify-center gap-2 mt-8">
            {rooms?.map((_, index) => <button key={index} className={`h-2 rounded-full transition-all ${index === current ? 'w-8 bg-primary' : 'w-2 bg-primary/30'}`} onClick={() => api?.scrollTo(index)} />)}
          </div>
        </div>
      </section>
      ...
      <VirtualTourViewer tourUrl={tourRoom?.virtual_tour_url || null} roomName={tourRoom?.name || ""} open={tourOpen} onOpenChange={setTourOpen} />
    </>;
};