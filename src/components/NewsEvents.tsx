...
const EventCard = ({
  ...
  return <div className="flex-[0_0_100%] min-w-0 sm:flex-[0_0_50%] lg:flex-[0_0_33.333%] xl:flex-[0_0_25%] px-2">
      <div className="bg-card rounded-lg shadow-lg overflow-hidden h-full flex flex-col transition-transform hover:scale-[1.02] duration-300">
        ...
        <div className="relative h-48 overflow-hidden">
          <img src={event.image_url || "/placeholder.svg"} alt={event.image_alt || event.name} className="w-full h-full object-cover" />
        </div>
...
export const NewsEvents = () => {
  ...
  if (isLoadingUpcoming) {
    return <section id="news-events" className="py-20 px-4 bg-secondary/30">
    ...
    return <section id="news-events" className="py-20 px-4 bg-secondary/30">
        <div className="container mx-auto max-w-7xl">
          <div className="text-center mb-12 animate-slide-up">
            ...
            <div className="h-1 w-24 bg-primary mx-auto mb-4 sm:mb-6" />
            <p className="text-sm sm:text-base md:text-lg text-muted-foreground max-w-2xl mx-auto">
              Temukan berbagai event menarik yang akan diselenggarakan di Semarang
            </p>
          </div>
  ...
  return <section id="news-events" className="py-20 px-4 bg-secondary/30">
      <div className="container mx-auto max-w-7xl">
        {/* Header */}
        <div className="text-center mb-12 animate-slide-up">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-4 sm:mb-6 font-serif lg:text-4xl">Agenda Seputar Semarang</h2>
          <div className="h-1 w-24 bg-primary mx-auto mb-4 sm:mb-6" />
          <p className="text-sm sm:text-base md:text-lg text-muted-foreground max-w-2xl mx-auto">
            Temukan berbagai event menarik yang akan diselenggarakan di Semarang
          </p>
        ...
        <div className="flex items-center justify-between mt-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              {scrollSnaps.map((_, index) => <button key={index} onClick={() => scrollTo(index)} className={`w-2.5 h-2.5 rounded-full transition-colors ${index === selectedIndex ? "bg-primary" : "bg-muted-foreground/30"}`} aria-label={`Go to slide ${index + 1}`} />)}
            </div>
          </div>
        </div>
      </div>
