    ...
    <>
      ...
      {!isOpen && (
        <Button
          ...
          className={cn(
            "fixed bottom-4 w-12 h-12 rounded-full shadow-lg z-50 transition-all hover:scale-110",
            position
          )}
      ...
      {isOpen && (
        <Card
          className={cn(
            "fixed bottom-4 w-[90vw] sm:w-80 h-[480px] max-w-sm shadow-2xl z-50 flex flex-col",
            position
          )}
        >
          {/* Header */}
          ...
          >
            <div className="flex items-center gap-2">
              <Avatar className={cn(
                "h-8 w-8",
                settings.bot_avatar_style === 'circle' ? 'rounded-full' :
                settings.bot_avatar_style === 'square' ? 'rounded-none' : 'rounded-md'
              )}>
