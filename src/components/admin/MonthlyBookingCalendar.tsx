...
import { format, eachDayOfInterval, getDay, addDays, parseISO, differenceInDays } from "date-fns";
  ...
  return (
    <>
      ...
      <Card className="w-full shadow-lg rounded-xl border-border/50">
        <div className="p-4 border-b border-border bg-slate-300">{/* CONTROL BAR (unchanged) */}</div>
        ...
        >
          <table className="w-full border-collapse">
            ...
            <tbody>{/* ROOM RENDERING — preserved, but cells below updated */}</tbody>
          </table>
        </div>
      </Card>
    </>
...
const BookingCell = ({ booking, visibleNights, isTruncatedLeft, onClick, CELL_WIDTH }) => {
  const widthPx = `${visibleNights * CELL_WIDTH}px`;
  ...
  return (
    <div
      ...
      style={{
        width: widthPx,
        left: isTruncatedLeft ? 0 : `${CELL_WIDTH / 2}px`,
      }}
    >
      {booking.guest_name}
    </div>
...
// 📌 UPDATED ROOM CELL
// ---------------------------
const RoomCell = ({ date, room, booking, onClick, CELL_WIDTH }) => {
