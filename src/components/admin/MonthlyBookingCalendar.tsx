  ...
  return <DndContext onDragEnd={handleDragEnd}>
        ...
        <div className="p-6 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 border-b border-gray-200">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Navigation Buttons */}
              <Button onClick={handlePrevMonth} variant="outline" size="sm" className="hover:bg-white/80 transition-all hover:shadow-md">
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <Button onClick={handleNextMonth} variant="outline" size="sm" className="hover:bg-white/80 transition-all hover:shadow-md">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead className="sticky top-0 z-20">
            <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
              ...
              {dates.map(date => {
                ...
                return <th key={date.toISOString()} className={`border border-gray-200 p-3 min-w-[70px] text-center transition-colors ${isWeekend ? "bg-amber-50/50" : "bg-white"}`}>
          ...
          <tbody>
            {Object.entries(roomsByType).map(([roomType, roomsInType], typeIndex) => <React.Fragment key={roomType}>
                {/* Room type header */}
                <tr className="border-y border-gray-200">
                  <td colSpan={dates.length + 1} className="p-3 bg-gradient-to-r from-gray-100 to-gray-50 font-bold text-sm uppercase tracking-wide text-gray-700">
                    {roomType}
                  </td>
                </tr>

                {/* Room rows */}
                {allRoomNumbers.filter(r => r.roomType === roomType).map((room, roomIndex) => <RoomRow key={room.roomNumber} room={room} roomIndex={roomIndex} dates={dates} getBookingForCell={getBookingForCell} isBookingStart={isBookingStart} isBookingEnd={isBookingEnd} isBeforeCheckout={isBeforeCheckout} isDateBlocked={isDateBlocked} handleBookingClick={handleBookingClick} />)}
              </React.Fragment>)}
          </tbody>
        </table>
      </div>
      ...
      <Dialog open={!!selectedBooking} onOpenChange={() => setSelectedBooking(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedBooking && <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      {selectedBooking.check_in_time && <p className="text-sm text-gray-600">{selectedBooking.check_in_time.slice(0, 5)}</p>}
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <Calendar className="h-5 w-5 text-purple-600 mt-0.5" />
                    <div>
                      ...
                      {selectedBooking.check_out_time && <p className="text-sm text-gray-600">
              ...
              {/* Special Requests */}
              {selectedBooking.special_requests && <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-lg p-4">
...
const DroppableRoomCell = ({
  ...
  return <td ref={setNodeRef} className={`border border-gray-100 p-0 relative h-16 min-w-[70px] transition-colors ${isWeekend ? "bg-amber-50/30" : ""} ${isOver ? "bg-blue-100 ring-2 ring-blue-400" : ""}`}>
      {/* Blocked Date Pattern */}
      {isBlocked && <div className="absolute inset-0 bg-gray-300" style={{
...
const RoomRow = ({
  ...
  return <tr ref={setNodeRef} className={`${roomIndex % 2 === 0 ? "bg-white" : "bg-gray-50/30"} hover:bg-blue-50/20 transition-colors ${isOver ? "bg-blue-50" : ""}`}>
      ...
      return <DroppableRoomCell key={date.toISOString()} roomNumber={room.roomNumber} date={date} booking={booking} isStart={isStart} isEnd={isEnd} showLCO={showLCO} isWeekend={isWeekend} isBlocked={isBlocked} handleBookingClick={handleBookingClick} />;
    })}
    </tr>;
};