import { Home, Calendar, Building2, ImageIcon, Boxes, Settings, MessageCircle, MapPin, CreditCard, Tags, RefreshCw, LayoutDashboard, Search, FileText, Compass, ChevronRight, Sparkles, Percent, TrendingUp, Bot } from "lucide-react";
...
export function AdminSidebar() {
  ...
  return <Sidebar collapsible={isMobile ? "offcanvas" : "icon"}>
      ...
      <SidebarContent className="px-2 overflow-y-auto flex-1">
        {menuGroups.map((group, groupIndex) => <SidebarGroup key={group.label} className="py-2">
            ...
            <SidebarGroupContent>
              ...
              return <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild tooltip={item.title} isActive={isActive}>
                        <NavLink to={item.url} end className={cn("flex items-center gap-3 rounded-lg transition-colors", isActive ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:text-foreground hover:bg-accent")} onClick={handleNavClick}>
