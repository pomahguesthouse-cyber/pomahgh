...
const FAQ_RE = /\b(fasilitas|facility|wifi|parkir|parking|sarapan|breakfast|kolam|pool|ac|handuk|towel|alamat|lokasi|location|arah|direction|dekat|nearby|jam\s+(?:buka|operasional|kerja)|buka\s+(?:jam|sampai)|tutup\s+(?:jam|pukul)|aturan|rule|policy|kebijakan|smoking|merokok|hewan|pet|anak|child|extra\s+bed|laundry|restoran|restaurant|mushola|masjid|transportasi|airport|bandara|stasiun|terminal|dapur|kitchen|pantry|masak|memasak|alat\s+masak|peralatan\s+masak|kompor|stove|microwave|kulkas|fridge|rice\s*cooker|panci|wajan|piring|gelas|sendok|garpu|cuci\s+piring)\b/i;
